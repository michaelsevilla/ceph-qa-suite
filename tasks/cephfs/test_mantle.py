...
#from tasks.teuthology import misc as teuthology
import logging
...
class TestMantle(CephFSTestCase):
    ...
    def test_dead_osd(self):
         #self.fs.mon_manager.raw_cluster_cmd_result(['osd', ])
         #x = self.fs.mon_manager.raw_cluster_cmd_result('osd')
         self.start_mantle()

         osd_map = json.loads(self.fs.mon_manager.raw_cluster_cmd('osd', 'dump', '--format=json-pretty'))
         osd_count = len(osd_map['osds'])
         print "OSD_COUNT", osd_count
         print "OSD_MAP", json.dumps(osd_map, indent=4, sort_keys=True)
         ...
         # put the balancer in there, so it registers... balance a couple times,
         # then kill the OSDs so that it knows its there but blocks and dies
         #lua_code = "BAL_LOG(0, \"test\")\nreturn {3, 4}"
         self.fs.mon_manager.raw_cluster_cmd_result('mds', 'set', 'balancer', "valid.lua")
         #self.fs.rados(["put", "valid.lua", "-"], stdin_data=lua_code)
         #with self.assert_cluster_log(success + "valid.lua"):
         ...
         #manager = tasks.ceph_manager.CephManager(
         #    mon,
         #    ctx=ctx,
